...
if (argc==2) {	strcpy(buf,argv[1]); ChangeFileExtension(buf,"TAP"); }
...
while(pos<flen-10)
	...
	switch(mem[pos-1])
		...
		case 0x10: len=Get2(&mem[pos+0x02]);
				   write(fho,&mem[pos+0x02],2); write(fho,&mem[pos+0x04],len);
				   pos+=len+0x04;
				   block++;
				   break;
...
printf("\n");
if (custom) printf("-- Warning: Custom Loading blocks were converted!\n");
if (longer) printf("-- Warning: Over 64k long Custom Loading blocks were *not* converted!\n");
if (only) printf("-- Warning: Some Pure Tone and/or Sequence of Pulses blocks encountered!\n");
if (dataonly) printf("-- Warning: Data Only blocks were converted!\n");
if (direct) printf("-- Warning: Direct Recording blocks were encountered!\n");
if (not_rec) printf("-- Warning: Some blocks were NOT recognised !\n");
